...
struct HufflepuffView: View {
    ...
    var body: some View {
        NavigationView{
            List{
                ForEach(0 ..< self.apiManager.hufChars.count) { (item) in
                    NavigationLink(destination: HufflepuffrowView(index: item)) {
                        HufflepuffcharList(index: item)
                    }
                }.frame(height: 200)
            }
...
struct HufflepuffcharList: View {
    @ObservedObject var apiManager = APIManager.shared
    var index: Int
    
    var body: some View {
        HStack (spacing: 30){
            URLImage(URL(string: self.apiManager.hufChars[index].image.replacingOccurrences(of: "http", with: "https"))!) { proxy in
                proxy.image
                    .resizable()
            ...
            Text(self.apiManager.hufChars[index].name)
                .font(.largeTitle)
                .fontWeight(.heavy)
...
struct HufflepuffrowView: View {
    ...
    var body: some View {
        ...
        VStack{
            if picshow{
                HufflepuffdetailView(index: self.index)
                    .transition(.slide)
            }
...
struct HufflepuffdetailView: View {
    @ObservedObject var apiManager = APIManager.shared
    var index: Int
    ...
    var body: some View {
        VStack (spacing: 30){
            ...
            HStack{
                ...
                Text(self.apiManager.hufChars[index].dateOfBirth)
                    .font(.system(size: 30))
                    .fontWeight(.bold)
                    .padding()
                    .foregroundColor(.black)
            ...
            HStack{
                ...
                Text(self.apiManager.hufChars[index].actor)
                    .font(.system(size: 30))
                    .fontWeight(.bold)
                    .padding()
                    .foregroundColor(.black)
