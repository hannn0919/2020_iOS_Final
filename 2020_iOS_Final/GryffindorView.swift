...
struct GryffindorView: View {
    ...
    var body: some View {
        NavigationView{
            List{
                ForEach(0 ..< self.apiManager.gryChars.count) { (item) in
                    NavigationLink(destination: GryffindorrowView(index: item)) {
                        GryffindorcharList(index: item)
                    }
                }.frame(height: 200)
            }
...
struct GryffindorcharList: View {
    @ObservedObject var apiManager = APIManager.shared
    var index: Int
    
    var body: some View {
        HStack (spacing: 30){
            URLImage(URL(string: self.apiManager.gryChars[index].image.replacingOccurrences(of: "http", with: "https"))!) { proxy in
                proxy.image
                    .resizable()
            ...
            Text(self.apiManager.gryChars[index].name)
                .font(.largeTitle)
                .fontWeight(.heavy)
...
struct GryffindorrowView: View {
    ...
    var body: some View {
        ...
        VStack{
            if picshow{
                GryffindordetailView(index: self.index)
                    .transition(.slide)
            }
...
struct GryffindordetailView: View {
    @ObservedObject var apiManager = APIManager.shared
    var index: Int
    ...
    var body: some View {
        VStack (spacing: 30){
            ...
            HStack{
                ...
                Text(self.apiManager.gryChars[index].dateOfBirth)
                    .font(.system(size: 30))
                    .fontWeight(.bold)
                    .padding()
                    .foregroundColor(.black)
            ...
            HStack{
                ...
                Text(self.apiManager.gryChars[index].actor)
                    .font(.system(size: 30))
                    .fontWeight(.bold)
                    .padding()
                    .foregroundColor(.black)
